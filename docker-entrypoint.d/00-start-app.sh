...
export S3_UPLOAD_BUCKET=`/envs/data_hub/bin/aws ssm get-parameters --name /apps/api_tnris_org/s3_upload_bucket --with-decryption --region us-east-1 --output text | awk '{print $6}'`
export S3_UPLOAD_SECRET=`/envs/data_hub/bin/aws ssm get-parameters --name /apps/api_tnris_org/s3_upload_secret --with-decryption --region us-east-1 --output text | awk '{print $6}'`
export S3_UPLOAD_KEY=`/envs/data_hub/bin/aws ssm get-parameters --name /apps/api_tnris_org/s3_upload_key --with-decryption --region us-east-1 --output text | awk '{print $6}'`
